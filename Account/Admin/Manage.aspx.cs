    ...
    protected void setPassword_Click(object sender, EventArgs e)
    {
        if (IsValid)
        {
            ...
            try
            {
                ...
                bool result = mu.ChangePassword(mu.ResetPassword(), NewPassword.Text );
                mu.UnlockUser();
                mu.IsApproved = true;
                Membership.UpdateUser(mu);
                if (result)
            {
                Response.Redirect("ListUsers?m=SetPwdSuccess");
            }
            }
            ...
            catch(Exception ex) 
            {
                ...
                newPasswordMessage.Text = "Error While updating " +ex.Message;
